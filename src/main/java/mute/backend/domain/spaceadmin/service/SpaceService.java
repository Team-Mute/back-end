package mute.backend.domain.spaceadmin.service;
...
import mute.backend.domain.spaceadmin.repository.AdminRegionRepository;
import mute.backend.domain.spaceadmin.repository.SpaceCategoryRepository;
import mute.backend.domain.spaceadmin.repository.SpaceImageRepository;
import mute.backend.domain.spaceadmin.repository.SpaceRepository;
import mute.backend.domain.spaceadmin.repository.SpaceTagMapRepository;
import mute.backend.domain.spaceadmin.repository.SpaceTagRepository;
import mute.backend.domain.spaceadmin.util.S3Deleter;
...
public class SpaceService {
  ...
  public Integer createWithImages(SpaceCreateRequest req, java.util.List<String> urls) {
    ...
    java.util.List<String> details =
        urls.size() > 1 ? urls.subList(1, urls.size()) : java.util.List.of();

    // 1. categoryName → categoryId
    SpaceCategory category =
        categoryRepository
            .findByCategoryName(req.getCategoryName())
            .orElseThrow(
                () -> new IllegalArgumentException("존재하지 않는 카테고리입니다: " + req.getCategoryName()));

    // 2. regionName → regionId
    AdminRegion region =
        regionRepository
            .findByRegionName(req.getRegionName())
            .orElseThrow(
                () -> new IllegalArgumentException("존재하지 않는 지역명입니다: " + req.getRegionName()));
      ...
      SpaceTag tag =
          tagRepository
              ...
              .orElseGet(
                  () -> {
                    SpaceTag newTag =
                        SpaceTag.builder().tagName(tagName).regDate(LocalDateTime.now()).build();
                    return tagRepository.save(newTag);
                  });

      SpaceTagMap map =
          SpaceTagMap.builder().space(saved).tag(tag).regDate(LocalDateTime.now()).build();
  ...
  public void updateWithImages(
      Integer spaceId, SpaceCreateRequest req, java.util.List<String> urls) {
    ...
    for (String tagName : req.getTagNames()) {
      ...
      SpaceTagMap map =
          SpaceTagMap.builder().space(space).tag(tag).regDate(LocalDateTime.now()).build();
      tagMapRepository.save(map);
    }
    ...
    List<String> oldGalleryUrls = oldImages.stream().map(SpaceImage::getImageUrl).toList();

    // 새 상세 이미지 URL 목록 (urls의 1번 인덱스부터)
    List<String> newGalleryUrls =
        (urls != null && urls.size() > 1)
