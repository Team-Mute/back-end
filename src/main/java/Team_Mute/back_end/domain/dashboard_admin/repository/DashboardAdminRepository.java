...
public interface DashboardAdminRepository extends JpaRepository<Reservation, Long> {
	 ...
	 * @param statusIds 캘린더에 표시할 예약 상태 ID 목록 (Integer 리스트)
	...
	@Query("SELECT r FROM Reservation r " +
		...
		"(r.reservationFrom BETWEEN :start AND :end OR r.reservationTo BETWEEN :start AND :end) AND " +
		// 상태 ID 필터링 (ReservationStatus 엔티티의 ID 사용)
		"rs.id IN :statusIds")
